...
class Node:
	...
	def insert_key_val(self, message):
		...
		reply = ""
		print('------->', self.nodeinfo, ' ...... ', reply)
		#print(succ)
		#print(self.nodeinfo)
		succ = succ.strip()
		ip,port = self.get_ip_port(succ)
		succ = NodeInfo(ip, port)
		if(succ.port == self.nodeinfo.port ):
			self.data_store.insert(data[0], data[1] )
			reply = succ
		else:
			reply = self.request_handler.send_message(succ.ip,succ.port,message)
		print('------->', self.nodeinfo, ' ...... ', reply)
		return reply
	...
	def search_key_val(self, message):
		...
		if(succ.port == self.nodeinfo.port ):
			print('FOUND')
			return self.data_store.search(key)
		else:
			print('SEARCHING REMOTELY...', str(succ))
			return self.request_handler.send_message(succ.ip, succ.port,message)
	...
	def delete_key_val(self, message):
		...
		if(succ.port == self.nodeinfo.port ):
			...
			succ = self.request_handler.send_message(succ.ip, succ.port,message)		
